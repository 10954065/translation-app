...
Primary provider is deep-translator's GoogleTranslator - the same free,
keyless Google Translate web endpoint that the academic reference
implementation's `googletrans` library also scrapes. Google's endpoint rate
limits aggressively on shared/datacenter IPs, so MyMemoryTranslator (a
separate free provider) is used as an automatic fallback whenever Google
fails. Callers never import deep-translator directly - only this file would
change to swap in Google Cloud Translation, DeepL, or another provider.
"""
...
class TranslationService:
    def __init__(self, timeout_seconds: float = 6.0, max_retries: int = 2):
    ...
    def translate(self, text: str, target_language: str, source_language: str) -> TranslationResult:
        ...
        translated = self._translate_with_google(text, source_language, target_language)
        fallback_used = False
        if translated is None:
            fallback_used = True
            translated = self._translate_with_mymemory(text, source_language, target_language)
